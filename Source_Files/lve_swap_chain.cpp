...
LveSwapChain::~LveSwapChain() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vkDestroySemaphore(device.device(), graphicsSemaphore, nullptr);
    vkDestroySemaphore(device.device(), computeSemaphore, nullptr);
}

void LveSwapChain::computeSubmitCommandBuffer(VkCommandBuffer computeCommandBuffer, uint32_t *imageIndex) {
    ...
    computeSubmitInfo.pWaitSemaphores = &imageAvailableSemaphores[*imageIndex];
    computeSubmitInfo.pWaitDstStageMask = &waitStageMask;
    computeSubmitInfo.signalSemaphoreCount = 1;
    computeSubmitInfo.pSignalSemaphores = &computeSemaphore;

    vkQueueSubmit(device.computeQueue_, 1, &computeSubmitInfo, VK_NULL_HANDLE);
}
...
VkResult LveSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex) {
    ...
    VkSemaphore graphicsSignalSemaphores[] = { renderFinishedSemaphores[*imageIndex] };
...
void LveSwapChain::createSyncObjects() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vkCreateSemaphore(device.device(), &semaphoreInfo, nullptr, &graphicsSemaphore);
    vkCreateSemaphore(device.device(), &semaphoreInfo, nullptr, &computeSemaphore);
}
